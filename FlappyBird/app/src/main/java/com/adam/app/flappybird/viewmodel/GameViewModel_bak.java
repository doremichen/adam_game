...
public class GameViewModel_bak extends AndroidViewModel {
    ...
    // Constructor
    public GameViewModel_bak(@NonNull Application application) {
        super(application);
        resetGame();
    ...
    private boolean isCollidingWithPipes(Pipe pipe) {
        boolean inXrange = mBird.getPosition().x + GameConstants.COLLISION_RANGE > pipe.getPosition().x
                && mBird.getPosition().x - GameConstants.COLLISION_RANGE < pipe.getRightX();
        boolean inYrange = mBird.getPosition().y + GameConstants.COLLISION_RANGE > pipe.getBottomPipeTopY()
                || mBird.getPosition().y - GameConstants.COLLISION_RANGE < pipe.getTopPipeBottomY();
